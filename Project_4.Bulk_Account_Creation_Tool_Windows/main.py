...
def run(address_set, cid_url, locale,url,Base_Account,Password,Account_suffix,value):
    ...
    try:
        ...
        if address_set == "y":
            ...
            #No need of setting address
            pass

        time.sleep(2)
       # driver.get("https://www.amazon.com/?portalDebug=1#Tasks|CUSTOMER_ID")
        driver.get(cid_url)
        time.sleep(5)
        cid = driver.find_element('xpath', "//span[contains(@id,'customer-id, CUSTOMER_ID')]").text
        cid=cid.split("[")[1].replace("]", "")
...
def myfunc():
    ...
    mydict = {}
    userChoice = 'y'
    address_set = input('Setting address for the accounts? - (y/n):  ')


    specific_choice = input('Want to create accounts only for any specific locale ?(y/n): ')
    if  specific_choice == 'n':
        while userChoice=='y':
        ...
        locale_specific = input('Enter the locale(us,de,uk,br,au,ca,it,jp,mx,es,fr,in,nl): ')
        Accounts_specific = input('Enter the Number of US Accounts Required in multiples of 4. e.g 4, 8, 12, 16: ')
    ...
    if specific_choice == "y":
        url1 = get_url(locale_specific)
        url = str(url1[0])
        cid_url = str(url1[10])
        print(cid_url)
        count = int(Accounts_specific) / 4
        initial = 0

        for k in range(int(count)):
            print()  # blank line
            print(f"Starting account creation for US locale - Iteration {k}")
            print('No of Accounts to be created - 4 ')
            ...
            # def run(locale,url,Base_Account,Password,Account_suffix,value) - for ref
            func = partial(run, address_set,cid_url, locale_specific, url, Base_Account, Password, Account_suffix)
        ...
        for locale in mydict:   #sequential loop
            ...
            pool = Pool(processes=10)
            #def run(locale,url,Base_Account,Password,Account_suffix,value) - for ref
            func = partial(run,address_set, cid_url, locale,url,Base_Account,Password,Account_suffix)
